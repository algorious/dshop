...
class CheckoutView(View):
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            order = Order.objects.get(user=self.request.user, ordered=False)
            # the following line prints requests in terminal - good for debugging
...
class OrdersCurrentUserView(LoginRequiredMixin, ListView):
    def get(self, *args, **kwargs):
        try:
            ...
            messages.info(self.request, "You did not have any orders")
            return redirect("/")


class OrdersCurrent_UserView(LoginRequiredMixin, ListView):
    model = Order
    template_name = 'orders.html'
    # paginate_by = 10

    def get_context_data(self, **kwargs):
        context = super(OrdersCurrent_UserView,
